...
class CrosswordCreator():
    ...
    def consistent(self, assignment):
        ...
        for var in assignment:
            # unary constraint (right length)
            if not len(assignment[var]) == var.length:
                return False
            # for every other variable in assignment
            for var2 in assignment:
                # check that every variable is different
                if var2 != var and assignment[var] == assignment[var2]:
                    return False
                if var2 in self.crossword.neighbors(var):
                    # check the ith element of var and jth of var2 are equal
                    x = self.crossword.overlaps[var, var2]
                    if x:
                        i, j = x
    ...
    def select_unassigned_variable(self, assignment):
        ...
        m = len(self.crossword.words) + 1
        best = None
        for var in self.crossword.variables:
            if var in assignment:
                continue
            if len(self.domains[var]) < m or len(self.domains[var]) == m and len(self.crossword.neighbors(var)) > len(self.crossword.neighbors(best)):
                best = var
                m = len(self.domains[var])
